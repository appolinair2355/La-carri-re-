...
function Message(type, text) {
  const colors = type === 'error' ? 'msg-error' : 'msg-success';
  const icon = type === 'error'
    ? '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>'
    : '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>';
  return `<div class="${colors}">${icon}${text}</div>`;
}
...
function renderLogin(container) {
  ...
  $('#loginForm').onsubmit = async (e) => {
    ...
    try {
      ...
      if (!result.success) {
        $('#msg').innerHTML = Message('error', result.error || 'Identifiants incorrects');
        return;
      }
      ...
      $('#msg').innerHTML = Message('error', 'Erreur de connexion');
    }
  };
}
...
function renderAdminLogin(container) {
  ...
  $('#adminForm').onsubmit = async (e) => {
    ...
    try {
      const result = await apiPost('/api/login', { identifier: email, password });
      if (!result.success || result.user.role !== 'admin') {
        $('#msg').innerHTML = Message('error', result.error || 'Accès refusé');
        return;
      }
  ...
  if (window.massierTab === 'rapport') {
      ...
      <div class="glass p-6">
        ...
        <form id="rapportForm" class="space-y-4">
          ...
          <div class="flex gap-3">
            ...
            <button type="submit" class="btn-small">Soumettre le rapport</button>
          </div>
        </form>
      </div>
    `;
    ...
    $('#rapportForm').onsubmit = async (e) => {
      ...
      try {
        ...
        if (!result.success) {
          $('#msg').innerHTML = Message('error', result.error || 'Erreur');
          return;
        }
        window.massierLignes = [{ nom_ouvrier: '', qte_515: '', qte_510: '' }];
    ...
    container.innerHTML = `
      <div class="glass p-6">
        ...
        <form id="venteForm" class="space-y-4 max-w-md">
          ...
          <button type="submit" class="btn-small">Déclarer la vente</button>
        </form>
      </div>
    `;
    ...
    $('#venteForm').onsubmit = async (e) => {
      ...
      const fd = new FormData(e.target);
      try {
        const result = await apiPost('/api/ventes', {
          massier_id: session.id,
          qte_515: parseInt(fd.get('qte_515')) || 0,
          qte_510: parseInt(fd.get('qte_510')) || 0
        });
        if (result.success) {
    ...
    container.innerHTML = `
      <div class="glass p-6">
        ...
        ${(db.validations_financieres || []).filter(v => v.massier_id === session.id).length === 0
          ? '<p class="text-gray-500 text-center py-8">Aucune validation reçue pour le moment.</p>'
          : `<div class="space-y-3">${(db.validations_financieres || []).filter(v => v.massier_id === session.id).map(v => `
...
function setProprioTab(tab) {
  ...
  tabs.forEach(t => {
    if (t.id && t.id.startsWith('ptab-')) t.className = 'tab tab-inactive';
  });
  const active = document.getElementById(`ptab-${tab}`);
  if (active) active.className = 'tab tab-active';
  renderProprioContent();
}
              ...
              ${ventes.map(v => {
                ...
                return `
                  <tr class="border-b border-gray-100">
                    ...
                    <td class="text-right py-3 px-2 font-medium">${v.qte_515 || 0}</td>
                    <td class="text-right py-3 px-2 font-medium">${v.qte_510 || 0}</td>
...
async function validateMassier(massierId) {
  try {
    ...
    const msgDiv = document.getElementById('msg');
    if (msgDiv) msgDiv.innerHTML = Message('error', 'Erreur de validation');
  }
}
...
async function validerRapport(rapportId, massierId) {
  ...
  try {
    const result = await apiPost('/api/valider-rapport', {
